...
class ToDoListApp:
    def __init__(self, root):
        ...
        label = tk.Label(text="to do list",fg="blue",bg="NavajoWhite",width=50,height=5,bd=5)
        label.config(font=('Helvatical bold',10))
        label.pack()
        self.tasks = []
        self.load_tasks()

        self.task_listbox = tk.Listbox(root,background="Pink",foreground="blue")
        self.task_listbox.pack(padx=10, pady=10, fill=tk.BOTH, expand=True)

        self.add_button = tk.Button(root, text="Add Task", command=self.add_task,background="coral2",foreground="Blue")
        self.update_button = tk.Button(root, text="Update Task", command=self.update_task,background="coral2",foreground="Blue")
        self.remove_button = tk.Button(root, text="Remove Task", command=self.remove_task,background="coral2",foreground="Blue")
        self.clear_button = tk.Button(root, text="Clear All", command=self.clear_tasks,background="coral2",foreground="Blue")
